...
class PhotobooksOverviewResponse(_PhotobooksOverviewResponse):
    ...
    unfulfilled_payment_ids: list[UUID]
    ...
    async def rendered_from_daos(
        ...
        # Payment
        unfulfilled_payment_ids_map: dict[UUID, list[UUID]] = defaultdict(list)
        for unfulfilled_payment_dao in unfulfilled_payment_daos:
            unfulfilled_payment_ids_map[unfulfilled_payment_dao.photobook_id].append(
                unfulfilled_payment_dao.id
            )
        ...
        for dao in daos:
            ...
            resp = cls(
                **dao.model_dump(),
                ...
                unfulfilled_payment_ids=unfulfilled_payment_ids_map.get(dao.id, []),
            )
